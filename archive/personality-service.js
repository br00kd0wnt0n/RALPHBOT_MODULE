...
class PersonalityService {
  ...
  async addQuirk(trigger, response) {
    try {
      ...
      let quirk = personality.quirks.find(q => q.trigger === trigger);
      if (quirk) {
        quirk.responses.push(response);
      } else {
      ...
      await this.updatePersonality(personality);
      return true;
    ...
    try {
      ...
      if (quirk) {
        ...
        if (quirk.responses.length === 0) {
          personality.quirks = personality.quirks.filter(q => q.trigger !== trigger);
        }
      }
...
export const initializePersonality = async () => {
  ...
  await service.initializePersonality();
  return service;
};

export const getPersonalityContext = async () => {
  const service = new PersonalityService();
  return await service.getPersonalityContext();
};
